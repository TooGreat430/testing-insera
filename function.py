...
def _backfill_zeroed_pl_numeric_from_recheck(rows: list, repaired_rows: list, vendor_id: str = "default"):
    ...
    # =====================================================================
    # PASS 2 — REKONSILIASI NW/GW NON-NOL (liow_ko, ber-guard).
    #
    # PASS 1 di atas hanya MENGISI field yang 0/null. Tapi kasus over-read
    # non-nol per baris (mis. PL tercetak 6.00 tapi terbaca 8.00) tidak
    # tersentuh: recheck liow_ko itu label-only dan backfill hanya isi nol.
    #
    # Di sini kita IZINKAN koreksi pl_nw / pl_gw NON-NOL dari nilai recheck,
    # TAPI hanya kalau koreksi itu MENGECILKAN selisih |sum baris - total
    # dokumen| (pl_total_nw / pl_total_gw). Guard ini membuat operasi AMAN:
    #  - tidak pernah memperburuk total PL (kalau menjauh -> ditolak),
    #  - tidak menyentuh baris yang sudah benar (mengubahnya justru menjauhkan
    #    total -> ditolak).
    # Trade-off yang disadari: karena guard-nya berbasis TOTAL, secara teori
    # baris benar bisa ter-clobber kalau kebetulan menurunkan gap agregat.
    # Itu sebabnya scope-nya sempit: hanya pl_nw/pl_gw, hanya baris PL nyata,
    # hanya vendor di PL_NUMERIC_RECHECK_BACKFILL_VENDORS.
    # =====================================================================
    GAP_EPSILON = 0.01
    for field, total_field in (("pl_nw", "pl_total_nw"), ("pl_gw", "pl_total_gw")):
        doc_total = None
        for row in rows:
            if not isinstance(row, dict):
                continue
            dt = _to_float(row.get(total_field))
        ...
        for row in rows:
            if isinstance(row, dict):
                cur_sum += _to_float(row.get(field)) or 0.0
        ...
        for row in rows:
            ...
            if row.get("_po_split_primary") is False:
                continue
            # hanya baris PL nyata (pl_quantity > 0)
            if (_to_float(row.get("pl_quantity")) or 0) <= 0:
                continue
            ...
            cur_val = _to_float(row.get(field)) or 0.0
            new_val = _to_float(repaired.get(field))
            if new_val is None or new_val < 0:
                continue
            if abs(new_val - cur_val) < GAP_EPSILON:
                continue

            old_gap = abs(cur_sum - doc_total)
            new_gap = abs((cur_sum - cur_val + new_val) - doc_total)
            if new_gap + GAP_EPSILON < old_gap:
                row[field] = new_val
                cur_sum = cur_sum - cur_val + new_val
                backfilled += 1
                print(
                    f"[PL_NUMERIC_RECONCILE] row_no={row_no} field={field} "
                    f"{cur_val} -> {new_val} (gap {old_gap:.2f} -> {new_gap:.2f})"
                )
