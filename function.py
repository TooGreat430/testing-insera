...
def _liow_ko_backfill_pl_rows_from_text_layer(rows: list, pl_parse: dict) -> int:
    """
    Isi sel pl_* yang ke-nol-kan/null oleh extraction (halaman PL terakhir
    tanpa header kolom membuat batch men-nol-kan QUANTITY/CTN/NW/GW) dengan
    nilai TERCETAK dari text layer PL yang sudah lolos gate sum==TOTAL.

    Append-only: hanya mengisi nilai 0/null — TIDAK pernah menimpa nilai
    non-zero hasil extraction. pl_package_count hanya diisi kalau memang
    tercetak (>0); sel CTN kosong pada baris lanjutan carton group tetap 0.
    Matching konsumtif urut dokumen by (item, qty) supaya duplikat genuine
    (item+qty sama 2x) tetap aman.
    """
    ...
    for row in rows:
        ...
        if not item_key and not _is_null(row.get("inv_spart_item_no")):
        ...
        def _fill_num(field, value):
            ...
            if cur is None or cur == 0:
                row[field] = _liow_ko_int_if_whole(v) if field in ("pl_quantity", "pl_package_count") else v
                row_filled += 1

        _fill_num("pl_quantity", pr.get("qty"))
        _fill_num("pl_nw", pr.get("nw"))
        _fill_num("pl_gw", pr.get("gw"))
        _fill_num("pl_package_count", pr.get("count"))
        ...
        if row_filled:
            filled_rows += 1
            filled_cells += row_filled

    if filled_cells:
        print(
            f"[LIOW_KO_PL_TEXT_BACKFILL] isi {filled_cells} sel pl_* tercetak "
            f"pada {filled_rows} baris dari text layer"
        )
    return filled_cells
...
def run_ocr(
    ...
    try:
        ...
        if normalize_vendor_id(vendor_id) == "liow_ko":
            ...
            print("[LIOW_KO_FIX_VERSION] text-layer-v1 aktif (deterministic count + index merge + printed totals + pl backfill)")
