...
def _doc_drop_prefixes(
    has_pl_doc: bool = True,
    has_bl_doc: bool = True,
    has_coo_doc: bool = True,
) -> tuple:
    """
    Prefix kolom yang HARUS dibuang dari output CSV detail kalau dokumen
    sumbernya tidak diupload. Kolom inv_*, po_*, match_*, confidence_label
    SELALU dipertahankan.

    Catatan prefix: "pl_" dan "po_" adalah prefix 3-karakter yang berbeda,
    jadi membuang "pl_" tidak akan menyentuh kolom po_*.
    """
    prefixes = []
    if not has_pl_doc:
        prefixes.append("pl_")
...
def _group_docs_by_invoice_no(invoice_paths, packing_paths, coo_paths=None, vendor_id: str = "default", require_packing: bool = True):
    ...
    for group_key, grp in groups.items():
        # PL hanya wajib kalau memang ada PL yang diupload (require_packing).
        # Mode invoice-only (require_packing=False) tidak boleh membuang group
        # hanya karena tidak punya packing.
...
# =========================================================================
# LIOW_KO: DETERMINISTIC TEXT-LAYER PARSE (pymupdf)
#
# PDF liow_ko (INV & PL) punya TEXT LAYER, jadi baris item + baris TOTAL
# tercetak bisa dibaca deterministik tanpa model — termasuk sel yang gagal
# dibaca Gemini dari render visual (halaman PL terakhir yang rapat/buram:
# sel TOTAL NW/GW, baris item tunggal setelah page break).
#
# Pendekatan GEOMETRIS (page.get_text("words") + koordinat), BUKAN regex per
# baris teks: urutan emisi text layer tidak stabil antar generator (format
# lama liow_ko mengeluarkan 1 sel per baris + kolom PO multi-nilai yang
# wrap, format baru 1 row per baris).
#   - Anchor baris = token PART NUMBER (kolomnya left-aligned -> mode x0).
#   - Band y antar-anchor membatasi 1 logical row.
#   - Token numerik ditugaskan ke kolom via x-center header (QUANTITY /
#     PRICE(USD) / AMOUNT untuk INV; QUANTITY / TOTAL CTN / NW / GW untuk PL),
#     difilter sejajar-y dengan anchor + tidak menempel kata kiri (angka
#     nyasar dari kalimat DESCRIPTION, mis. "ALLOY 6061").
#   - Kolom PO = semua kata di kiri kolom PART (digabung; menangani PO
#     multi-nilai "45326657/45327769/..." yang wrap lintas baris).
#
# GATE SELF-CONSISTENCY (anti salah parse, zero blast radius):
#   INV: tiap baris qty x price == amount, DAN sum(qty)/sum(amount) ==
#        TOTAL tercetak.
#   PL : sum(qty)/sum(ctn)/sum(nw)/sum(gw) == keempat TOTAL tercetak.
# Kalau gate gagal (scan tanpa text layer, layout tak dikenal) -> return {}
# dan SEMUA pemakaian di-skip: perilaku persis seperti sebelum fix.
# =========================================================================

_LIOW_KO_PART_TOKEN_RE = re.compile(r"^[A-Z][A-Z0-9]{7,}$")
_LIOW_KO_NUM_TOKEN_RE = re.compile(r"^(?:\d{1,3}(?:,\d{3})*(?:\.\d+)?|\d+(?:\.\d+)?|\.\d+)$")
_LIOW_KO_UNIT_TOKEN_RE = re.compile(r"^(SET|PCS|PRS)$")
# Baris TOTAL liow_ko bisa tercetak dengan spasi di dalam angka ("1, 111. 24")
_LIOW_KO_SPACED_NUM_RE = re.compile(
    r"\d{1,3}(?:\s?,\s?\d{3})+(?:\s?\.\s?\d{1,2})?|\d+(?:\s?\.\s?\d{1,2})?|\.\d{1,2}"
)


def _liow_ko_num(token):
    s = re.sub(r"\s", "", str(token)).replace(",", "")
    if s.startswith("."):
        s = "0" + s
    try:
        return float(s)
...
def _liow_ko_find_part_column_band(pages_words):
    # Kolom PART NUMBER left-aligned -> mode x0 dari kandidat token part.
    candidates = []
    for words in pages_words:
        for w in words:
            t = w["text"]
            if _LIOW_KO_PART_TOKEN_RE.match(t) and any(c.isdigit() for c in t):
                candidates.append(w)
    if not candidates:
        return None
    freq = {}
    for w in candidates:
        key = int(round(w["x0"] / 3.0))
        freq[key] = freq.get(key, 0) + 1
    best_key = max(freq, key=lambda k: freq[k])
    xs = [w["x0"] for w in candidates if int(round(w["x0"] / 3.0)) == best_key]
    return (min(xs) - 3.0, max(xs) + 3.0)


def _liow_ko_collect_anchors(words, part_band):
    lo, hi = part_band
    anchors = [
        w for w in words
        if _LIOW_KO_PART_TOKEN_RE.match(w["text"])
        and any(c.isdigit() for c in w["text"])
        and lo <= w["x0"] <= hi
    ]
    anchors.sort(key=lambda w: w["yc"])
    return anchors


def _liow_ko_median_anchor_gap(pages_anchors, fallback=40.0):
    gaps = []
    for anchors in pages_anchors:
...
def _liow_ko_header_col_centers(pages_words, labels):
    centers = {}
    for words in pages_words:
        for w in words:
            t = w["text"].upper().rstrip(":")
            if t in labels and labels[t] not in centers:
                centers[labels[t]] = w["xc"]
        if len(centers) == len(set(labels.values())):
            break
    return centers
...
def _liow_ko_assign_numeric_to_columns(band_words, col_centers, part_band_hi, anchor_yc):
    ...
    # DESCRIPTION:
    #   1) nilai sel di-center vertikal per row -> wajib sejajar y dengan anchor
    ...
    for w in band_words:
        if w["xc"] <= part_band_hi:
            continue
        if not _LIOW_KO_NUM_TOKEN_RE.match(w["text"]):
            continue
        if abs(w["yc"] - anchor_yc) > 4.5:
            continue
        attached_left = any(
            nb is not w
            and abs(nb["yc"] - w["yc"]) < 3.0
            and 0 <= (w["x0"] - nb["x1"]) < 8.0
            and not _LIOW_KO_NUM_TOKEN_RE.match(nb["text"])
            for nb in band_words
        )
        if attached_left:
            continue
        candidates.append(w)
...
def _liow_ko_parse_doc_text_layer(pdf_path: str, doc_kind: str) -> dict:
    """
    doc_kind: "inv" | "pl".
    Return:
      inv -> {"rows": [{po,item,unit,qty,price,amount,page,page_index}],
              "total_quantity", "total_amount"}
      pl  -> {"rows": [{po,item,unit,qty,count,nw,gw,page,page_index}],
              "total_quantity", "total_package", "total_nw", "total_gw"}
    {} kalau text layer absen / parse tidak lolos gate self-consistency.
    """
    try:
        doc = fitz.open(pdf_path)
        try:
            pages_words = [_liow_ko_page_words(p) for p in doc]
        finally:
            doc.close()
    except Exception as e:
        print(f"[LIOW_KO_TEXT_LAYER][{doc_kind}] gagal baca pdf: {e}")
        return {}

    if not any(pages_words):
        return {}

    part_band = _liow_ko_find_part_column_band(pages_words)
    if not part_band:
        return {}

    pages_anchors = [_liow_ko_collect_anchors(words, part_band) for words in pages_words]
    if not any(pages_anchors):
        return {}
    median_gap = _liow_ko_median_anchor_gap(pages_anchors)

    if doc_kind == "inv":
        col_centers = _liow_ko_header_col_centers(
            pages_words,
            {"QUANTITY": "qty", "PRICE(USD)": "price", "AMOUNT": "amount"},
        )
        required_cols = {"qty", "price", "amount"}
    else:
        col_centers = _liow_ko_header_col_centers(
            pages_words,
            {"QUANTITY": "qty", "TOTAL": "count", "NW": "nw", "GW": "gw"},
        )
        required_cols = {"qty", "nw", "gw"}

    if not required_cols.issubset(col_centers.keys()):
        print(
            f"[LIOW_KO_TEXT_LAYER][{doc_kind}] header kolom tidak lengkap: "
            f"{sorted(col_centers)}"
        )
        return {}
    ...
    def _parse_rows(bottom_cut_factor, fail):
        ...
        for page_idx, (words, anchors) in enumerate(zip(pages_words, pages_anchors)):
            ...
            for i, a in enumerate(anchors):
                top = (anchors[i - 1]["yc"] + a["yc"]) / 2.0 if i > 0 else a["yc"] - median_gap * 0.55
                bot = (
                    (a["yc"] + anchors[i + 1]["yc"]) / 2.0
                    if i < len(anchors) - 1
                    else a["yc"] + median_gap * bottom_cut_factor
                )
                bounds.append((top, bot))

            for i, a in enumerate(anchors):
                top, bot = bounds[i]
                band_words = [w for w in words if top <= w["yc"] < bot]

                if doc_kind == "inv":
                    total_label = [w for w in band_words if w["text"].upper().startswith("TOTAL")]
                    if total_label:
                        cut_y = min(w["yc"] for w in total_label) - 2.0
                        band_words = [w for w in band_words if w["yc"] < cut_y]

                assigned = _liow_ko_assign_numeric_to_columns(
                ...
                unit_tokens = [
                    ...
                    if _LIOW_KO_UNIT_TOKEN_RE.match(w["text"].upper())
                ]
                unit = (
                    max(unit_tokens, key=lambda w: w["xc"])["text"].upper()
                    if unit_tokens else "null"
                )

                po_words = sorted(
                    (w for w in band_words if w["x0"] < part_band[0] - 1.0),
                    key=lambda w: (w["yc"], w["x0"]),
                )
                po_text = "".join(w["text"] for w in po_words).strip()
                ...
                row.update({k: (v if v is not None else 0.0) for k, v in assigned.items()})
                rows.append(row)

            if page_idx == last_page_idx:
                bot_last = bounds[-1][1]
                total_region_words = sorted(
                    (w for w in words if w["yc"] >= bot_last),
                    key=lambda w: (w["yc"], w["x0"]),
                )
    ...
    for cut_factor in (0.55, 0.4, 0.75):
        ...
        if packed:
            ...
            print(
                f"[LIOW_KO_TEXT_LAYER][{doc_kind}] OK rows={len(packed['rows'])} "
                f"cut={cut_factor} totals={printed}"
            )
            return packed
...
def _liow_ko_int_if_whole(value):
    try:
        f = float(value)
    except (TypeError, ValueError):
        return value
...
def _liow_ko_merge_index_with_text_layer_rows(index_items: list, inv_parse: dict, pl_parse: dict = None) -> list:
    """
    Sisipkan line item yang TERBUKTI tercetak di invoice (text layer, lolos
    gate sum==TOTAL) tapi hilang dari index Gemini — kasus produksi: item
    tunggal di halaman terakhir setelah page break tidak diindeks sehingga
    total qty/amount kurang persis sebesar baris itu.

    Konservatif:
    - Matching MULTISET by key (item, qty, amount) — genuine duplicate aman.
    - HANYA jalan kalau SEMUA index item Gemini ter-match ke baris tercetak.
      Kalau ada index item yang tidak match (indikasi misread/ghost), index
      dikembalikan APA ADANYA (zero behavior change).
    - Urutan hasil = urutan dokumen (urutan parse text layer).
    """
    parsed_rows = (inv_parse or {}).get("rows") or []
    if not parsed_rows or not isinstance(index_items, list) or not index_items:
        return index_items
    if len(index_items) >= len(parsed_rows):
        return index_items
    ...
    def _idx_key(it):
        if not isinstance(it, dict):
            return None
        return (
            _norm_item_compare_key(it.get("inv_spart_item_no")),
            round(_to_float(it.get("inv_quantity")) or 0.0, 2),
            round(_to_float(it.get("inv_amount")) or 0.0, 2),
        )

    def _parsed_key(pr):
        return (
            _norm_item_compare_key(pr.get("item")),
            round(pr.get("qty") or 0.0, 2),
            round(pr.get("amount") or 0.0, 2),
        )

    pool = {}
    for pos, it in enumerate(index_items):
        pool.setdefault(_idx_key(it), []).append(pos)

    pl_by_key = {}
    for pr in (pl_parse or {}).get("rows") or []:
        key = (_norm_item_compare_key(pr.get("item")), round(pr.get("qty") or 0.0, 2))
        pl_by_key.setdefault(key, pr)
    ...
    for pr in parsed_rows:
        k = _parsed_key(pr)
        positions = pool.get(k)
        if positions:
            merged.append(index_items[positions.pop(0)])
            consumed += 1
            continue

        pl_row = pl_by_key.get((k[0], k[1])) or {}
        synth = {
            "idx": 0,
            "page": pr.get("page") or 0,
            "page_index": pr.get("page_index") or 0,
            "inv_customer_po_no": pr.get("po") or "null",
            "inv_spart_item_no": pr.get("item") or "null",
            "inv_description": "null",
            "inv_quantity": _liow_ko_int_if_whole(pr.get("qty") or 0),
            "inv_quantity_unit": pr.get("unit") or "null",
            "inv_unit_price": pr.get("price") or 0,
            "inv_price_unit": "null",
            "inv_amount": pr.get("amount") or 0,
            "pl_page_no": pl_row.get("page") or 0,
            "pl_customer_po_no": pl_row.get("po") or "null",
            "pl_description": "null",
            "pl_quantity": _liow_ko_int_if_whole(pl_row.get("qty") or 0),
        }
        merged.append(synth)
        synth_rows.append(pr)

    if consumed != len(index_items):
        print(
            f"[LIOW_KO_INDEX_MERGE] skip: {len(index_items) - consumed} index "
            f"item tidak match baris tercetak (kemungkinan misread) — index "
            f"dibiarkan apa adanya"
        )
        return index_items
    ...
    for pr in synth_rows:
        print(
            f"[LIOW_KO_INDEX_MERGE] tambah line item tercetak yang hilang dari "
            f"index: po={pr.get('po')} item={pr.get('item')} "
            f"qty={pr.get('qty')} amount={pr.get('amount')} (page {pr.get('page')})"
        )
    print(
        f"[LIOW_KO_INDEX_MERGE] index: {len(index_items)} -> {len(merged)} item"
    )
    return merged


def _liow_ko_backfill_pl_rows_from_text_layer(rows: list, pl_parse: dict) -> int:
    """
    Samakan sel pl_* numerik dengan nilai TERCETAK dari text layer PL yang
    sudah lolos gate sum==TOTAL (lihat _liow_ko_parse_doc_text_layer):

    1) BACKFILL: isi sel 0/null yang ke-nol-kan extraction (halaman PL
       terakhir tanpa header kolom membuat batch men-nol-kan QUANTITY/CTN/
       NW/GW). pl_package_count hanya diisi kalau memang tercetak (>0);
       sel CTN kosong pada baris lanjutan carton group tetap 0.
    2) KOREKSI: timpa nilai non-zero yang BEDA dari nilai tercetak. Halaman
       PL terakhir yang rapat membuat model salah baca sel NW/GW (salin
       nilai baris tetangga / salah digit: 0.87->4.44, 6.20->6.28,
       35.60->36.00 pada LK-A20260522001). Aman menimpa karena baris parse
       TERBUKTI konsisten: sum per-baris == keempat TOTAL tercetak.
       Termasuk men-NOL-kan pl_package_count yang dihalusinasi pada sel CTN
       yang tercetak kosong, supaya sigma CTN == TOTAL CTN tercetak.

    Matching konsumtif urut dokumen by (item, qty) supaya duplikat genuine
    (item+qty sama 2x) tetap aman. Baris yang qty-nya tidak match dicoba
    ulang by item-only HANYA kalau tidak ambigu (sisa kandidat parse DAN
    baris unmatched untuk item itu sama-sama tunggal) — menangkap kasus
    qty misread tanpa risiko salah pasang duplikat.
    """
    parsed = (pl_parse or {}).get("rows") or []
    if not parsed or not isinstance(rows, list):
        return 0

    pool = {}
    for pr in parsed:
        key = (_norm_item_compare_key(pr.get("item")), round(pr.get("qty") or 0.0, 2))
        pool.setdefault(key, []).append(pr)

    touched_cells = 0
    touched_rows = 0

    def _row_item_key(row):
        # _norm_item_compare_key("null") menghasilkan "NULL" (bukan string
        # kosong), jadi nilai null harus disaring DULU sebelum dinormalisasi.
        if not _is_null(row.get("pl_item_no")):
            return _norm_item_compare_key(row.get("pl_item_no"))
        if not _is_null(row.get("inv_spart_item_no")):
            return _norm_item_compare_key(row.get("inv_spart_item_no"))
        return ""

    def _sync_row_with_parsed(row, pr, item_key):
        row_touched = 0

        def _sync_num(field, value, allow_zero_out=False):
            nonlocal row_touched
            v = _to_float(value)
            cur = _to_float(row.get(field))
            if v is None:
                return
            if v <= 0:
                # Sel tercetak kosong. Nilai extraction >0 di sel kosong =
                # halusinasi (gate menjamin parse benar) -> nolkan.
                if allow_zero_out and cur is not None and cur > 0:
                    print(
                        f"[LIOW_KO_PL_TEXT_CORRECT] {field} "
                        f"(item={item_key}): {cur} -> 0 (sel tercetak kosong)"
                    )
                    row[field] = 0
                    row_touched += 1
                return
            new_val = (
                _liow_ko_int_if_whole(v)
                if field in ("pl_quantity", "pl_package_count")
                else v
            )
            if cur is None or cur == 0:
                ...
                print(
                    f"[LIOW_KO_PL_TEXT_CORRECT] {field} "
                    f"(item={item_key}): {cur} -> {new_val} (nilai tercetak)"
                )
                row[field] = new_val
                row_touched += 1

        _sync_num("pl_quantity", pr.get("qty"))
        _sync_num("pl_nw", pr.get("nw"))
        _sync_num("pl_gw", pr.get("gw"))
        _sync_num("pl_package_count", pr.get("count"), allow_zero_out=True)

        if (_to_float(row.get("pl_package_count")) or 0) > 0 and _is_null(row.get("pl_package_unit")):
            row["pl_package_unit"] = "CT"
            row_touched += 1
        if _is_null(row.get("pl_item_no")) and pr.get("item"):
            row["pl_item_no"] = pr["item"]
            row_touched += 1
        if _is_null(row.get("pl_customer_po_no")) and pr.get("po"):
            row["pl_customer_po_no"] = pr["po"]
            row_touched += 1
        return row_touched

    # PASS 1: matching by (item, qty) — qty dari pl_quantity lalu inv_quantity.
    unmatched_rows = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        ...
        qty_candidates = []
        for src in (row.get("pl_quantity"), row.get("inv_quantity")):
            q = _to_float(src)
            if q and q > 0 and round(q, 2) not in qty_candidates:
                qty_candidates.append(round(q, 2))

        pr = None
        for qc in qty_candidates:
            bucket = pool.get((item_key, qc))
            if bucket:
                pr = bucket.pop(0)
                break
        if pr is None:
            unmatched_rows.append((row, item_key))
            continue

        row_touched = _sync_row_with_parsed(row, pr, item_key)
        if row_touched:
            touched_rows += 1
            touched_cells += row_touched

    # PASS 2: item-only untuk baris yang qty-nya tidak match (qty misread).
    # Hanya kalau TIDAK ambigu: sisa kandidat parse untuk item itu tunggal
    # DAN baris unmatched untuk item itu juga tunggal.
    leftover_by_item = {}
    for (ik, _qc), bucket in pool.items():
        for pr in bucket:
            leftover_by_item.setdefault(ik, []).append(pr)
    unmatched_by_item = {}
    for row, ik in unmatched_rows:
        unmatched_by_item.setdefault(ik, []).append(row)

    for ik, candidates in leftover_by_item.items():
        rows_for_item = unmatched_by_item.get(ik) or []
        if len(candidates) != 1 or len(rows_for_item) != 1:
            continue
        row_touched = _sync_row_with_parsed(rows_for_item[0], candidates[0], ik)
        if row_touched:
            touched_rows += 1
            touched_cells += row_touched
    ...
    if touched_cells:
        print(
            f"[LIOW_KO_PL_TEXT_BACKFILL] sinkron {touched_cells} sel pl_* "
            f"tercetak pada {touched_rows} baris dari text layer"
        )
...
def _map_po_to_details(po_lines, detail_rows, vendor_id="default"): # <-- Jangan lupa param vendor_id
    ...
    for row in detail_rows or []:
        ...
        if inv_po_norm and art_to_check:
            ...
            if not exists_in_po:
                ...
                if pl_po_norm and pl_po_norm == inv_po_norm:
                    # KONSENSUS inv==pl: PO yang sama tercetak di Invoice DAN
                    # Packing List untuk baris ini — bukan warisan fill_forward
                    # (fill_forward hanya menyentuh kolom invoice). Item tidak
                    # ada di PO itu pada master -> biarkan unmapped ("PO item
                    # tidak ditemukan") daripada meminjam nomor PO lain yang
                    # tidak pernah tercetak di dokumen.
                    print(
                        f"[PO_CONSENSUS_GUARD] keep extracted PO "
                        f"'{row.get('inv_customer_po_no')}' "
                        f"(article={row.get('inv_spart_item_no')}): invoice & PL "
                        f"mencetak PO yang sama; tidak ditimpa PO tebakan."
                    )
                elif inv_po_norm in doc_anchored_po_norms:
    ...
    for i, original_row in enumerate(detail_rows or []):
        ...
        # KONSENSUS inv==pl: baris ini punya PO sendiri yang tercetak konsisten
        # di Invoice & Packing List — gagal map artinya PO/item memang tidak ada
        # di PO master. Jangan pinjam PO tetangga: hasilnya PO yang tidak pernah
        # tercetak di dokumen + false po_price mismatch. Neighbor fallback hanya
        # untuk baris yang PO-nya gagal terbaca / tidak dicetak per baris.
        own_inv_po_norm = _norm_po_number(original_row.get("inv_customer_po_no"))
        own_pl_po_norm = _norm_po_number(original_row.get("pl_customer_po_no"))
        if own_inv_po_norm and own_inv_po_norm == own_pl_po_norm:
            print(
                f"[PO_CONSENSUS_GUARD] PO tercetak "
                f"'{original_row.get('inv_customer_po_no')}' "
                f"(article={original_row.get('inv_spart_item_no')}) tidak "
                f"ditemukan di PO master — biarkan unmapped, jangan warisi PO "
                f"tetangga."
            )
            continue
...
def _enforce_absent_optional_docs_empty(
    ...
    *,
    ...
    Rule:
    - has_pl_doc=False  -> semua pl_* = 'null'
...
def _convert_to_csv_path(blob_path, rows, field_order=None, drop_prefixes=None):
    ...
    # Buang kolom milik dokumen yang tidak diupload (mis. bl_/coo_/pl_).
    # Diterapkan setelah keys final supaya extra-key dari union_keys juga
    # ikut ter-drop, bukan cuma yang ada di field_order.
    if drop_prefixes:
        dp = tuple(drop_prefixes)
        keys = [k for k in keys if not str(k).startswith(dp)]
...
def run_grouped_ocr(invoice_name, uploaded_docs, with_total_container, forced_vendor_id=None):
    ...
    # Hanya INVOICE yang wajib. PL opsional (mode invoice-only).
    ...
    try:
        groups = _group_docs_by_invoice_no(
            ...
            # Kalau PL tidak diupload, jangan buang group invoice yang tidak
            # punya packing.
            require_packing=has_pl_doc_global,
        )
        ...
        for _, grp in sorted(groups.items(), key=lambda item: str(item[1]["invoice_no"])):
            ...
            try:
                ...
                # PL per group (opsional). Urutan input WAJIB: [invoice, packing?, BL?, COO?]
        ...
        detail_csv_uri = _convert_to_csv_path(
            ...
            # Kolom menyesuaikan dokumen yang diupload di level run:
            # pl_* hanya kalau ada PL, bl_* kalau ada BL, coo_* kalau ada COO.
            drop_prefixes=_doc_drop_prefixes(
...
def run_ocr(
    ...
    explicit_has_pl_doc = has_pl_doc is not None
    ...
    # has_pl_doc default True supaya caller legacy (yang selalu kirim INV+PL)
    # tidak berubah perilakunya.
    if has_pl_doc is None:
        has_pl_doc = len(uploaded_pdf_paths) >= 2
    ...
    try:
        ...
        if has_pl_doc and len(normalized_pdf_paths) < 2:
            raise Exception("has_pl_doc=True tapi packing list tidak ada di input.")

        # ==========================================
        # POSITIONAL SLOT MAP
        # ==========================================
        # Urutan input: [invoice, packing?, BL?, COO?] — slot dinamis sesuai
        # dokumen yang benar-benar diupload. Index dihitung berurutan supaya
        # mode invoice-only / inv+bl (tanpa PL) tetap menunjuk file yang benar.
        invoice_idx = 0
        _next_idx = 1
        packing_idx = None
        if has_pl_doc:
            packing_idx = _next_idx
            _next_idx += 1
        bl_idx = None
        if has_bl_doc and len(normalized_pdf_paths) > _next_idx:
            bl_idx = _next_idx
            _next_idx += 1
        coo_idx = None
        if has_coo_doc and len(normalized_pdf_paths) > _next_idx:
            coo_idx = _next_idx
            _next_idx += 1

        print(
            f"[DOC_SLOTS] has_pl_doc={has_pl_doc} has_bl_doc={has_bl_doc} "
            f"has_coo_doc={has_coo_doc} invoice_idx={invoice_idx} "
            f"packing_idx={packing_idx} bl_idx={bl_idx} coo_idx={coo_idx}"
        )
        ...
        # packing_onepage_pdf = None kalau PL tidak diupload (mode invoice-only).
        ...
        if _skip_onepage_preprocess:
            ...
            invoice_onepage_pdf = normalized_pdf_paths[invoice_idx]
            if packing_idx is not None:
                packing_onepage_pdf = normalized_pdf_paths[packing_idx]
        else:
        ...
        # Detail input = invoice (+ packing kalau ada).
        preprocessed_detail_inputs = [invoice_onepage_pdf]
        if packing_onepage_pdf is not None:
            preprocessed_detail_inputs.append(packing_onepage_pdf)
        ...
        extra_doc_indices = [i for i in (bl_idx, coo_idx) if i is not None]
        has_extra_docs = bool(extra_doc_indices) and (has_bl_doc or has_coo_doc)
        if has_extra_docs:
            full_input_paths = list(preprocessed_detail_inputs) + [
                normalized_pdf_paths[i] for i in extra_doc_indices
            ]
        ...
        if has_bl_doc and bl_idx is not None:
            bl_only_pdf = _compress_pdf_if_needed(normalized_pdf_paths[bl_idx])
            if bl_only_pdf not in temp_local_paths and bl_only_pdf != normalized_pdf_paths[bl_idx]:
        ...
        if has_coo_doc and coo_idx is not None:
            coo_only_pdf = _compress_pdf_if_needed(normalized_pdf_paths[coo_idx])
            if coo_only_pdf not in temp_local_paths and coo_only_pdf != normalized_pdf_paths[coo_idx]:
        ...
        if with_total_container:
            if not has_bl_doc or bl_idx is None:
                raise Exception("Output total/container membutuhkan dokumen Bill of Lading.")

            bl_pdf_for_container = _compress_pdf_if_needed(normalized_pdf_paths[bl_idx])
            if (
                bl_pdf_for_container not in temp_local_paths
                and bl_pdf_for_container != normalized_pdf_paths[bl_idx]
            ):
        ...
        # =========================================
        # LIOW_KO: DETERMINISTIC TEXT-LAYER PARSE (pymupdf).
        # Hasil parse hanya dipakai kalau lolos gate self-consistency
        # (sum per-baris == TOTAL tercetak); gagal -> {} dan semua pemakaian
        # di bawah otomatis ter-skip (perilaku lama). Lihat
        # _liow_ko_parse_doc_text_layer.
        # Dipakai untuk:
        #  1) override header total dengan nilai TERCETAK (halaman PL terakhir
        #     yang rapat membuat model salah baca sel TOTAL NW/GW),
        #  2) override total_row (count baris deterministik),
        #  3) menyisipkan line item tercetak yang hilang dari index,
        #  4) backfill sel pl_* yang ke-nol-kan pada halaman PL tanpa header.
        # =========================================
        liow_ko_inv_parse = {}
        liow_ko_pl_parse = {}
        if normalize_vendor_id(vendor_id) == "liow_ko":
            # Marker versi untuk verifikasi deploy (cek log run produksi).
            print("[LIOW_KO_FIX_VERSION] text-layer-v2 aktif (deterministic count + index merge + printed totals + pl backfill&correct)")
            liow_ko_inv_parse = _liow_ko_parse_doc_text_layer(
                normalized_pdf_paths[invoice_idx], "inv"
            )
            if packing_idx is not None:
                liow_ko_pl_parse = _liow_ko_parse_doc_text_layer(
                    normalized_pdf_paths[packing_idx], "pl"
                )

            liow_ko_header_overrides = []
            if liow_ko_inv_parse:
                liow_ko_header_overrides += [
                    ("inv_total_quantity", liow_ko_inv_parse.get("total_quantity")),
                    ("inv_total_amount", liow_ko_inv_parse.get("total_amount")),
                ]
            if liow_ko_pl_parse:
                liow_ko_header_overrides += [
                    ("pl_total_quantity", liow_ko_pl_parse.get("total_quantity")),
                    ("pl_total_package", liow_ko_pl_parse.get("total_package")),
                    ("pl_total_nw", liow_ko_pl_parse.get("total_nw")),
                    ("pl_total_gw", liow_ko_pl_parse.get("total_gw")),
                ]
            for _field, _printed in liow_ko_header_overrides:
                if _printed is None or _printed <= 0:
                    continue
                _printed = _liow_ko_int_if_whole(_printed)
                if _to_float(base_header_obj.get(_field)) != _to_float(_printed):
                    print(
                        f"[LIOW_KO_PRINTED_TOTAL] header {_field}: "
                        f"{base_header_obj.get(_field)} -> {_printed} "
                        f"(nilai tercetak, text layer)"
                    )
        ...
        # LIOW_KO: override total_row dengan jumlah baris deterministik dari
        # text layer (lolos gate sum==TOTAL tercetak). Row count Gemini bisa
        # undercount (item tunggal halaman terakhir setelah page break tidak
        # terhitung) dan angka itu MENULAR ke index ("array harus = total_row").
        if normalize_vendor_id(vendor_id) == "liow_ko" and liow_ko_inv_parse.get("rows"):
            liow_ko_deterministic_total_row = len(liow_ko_inv_parse["rows"])
            if liow_ko_deterministic_total_row != total_row:
                print(
                    f"[LIOW_KO_PART_COUNT] override total_row: "
                    f"gemini={total_row} -> text_layer={liow_ko_deterministic_total_row}"
                )
                total_row = liow_ko_deterministic_total_row
        ...
        # LIOW_KO: sisipkan line item yang TERBUKTI tercetak di invoice
        # (text layer lolos gate sum==TOTAL) tapi hilang dari index — kasus
        # produksi LK-A20260522001: item tunggal halaman 4 (626 SET /
        # 1,965.64) tidak diindeks -> total qty/amount mismatch persis
        # sebesar baris itu. Konservatif: hanya jalan kalau SEMUA index item
        # Gemini ter-match ke baris tercetak; kalau tidak, index dibiarkan.
        if normalize_vendor_id(vendor_id) == "liow_ko" and liow_ko_inv_parse.get("rows"):
            index_items = _liow_ko_merge_index_with_text_layer_rows(
                index_items,
                liow_ko_inv_parse,
                liow_ko_pl_parse,
            )
        ...
        # =========================================
        # LIOW_KO: backfill sel pl_* yang ke-nol-kan extraction (halaman PL
        # terakhir tanpa header kolom) dengan nilai TERCETAK dari text layer
        # (lolos gate sum==TOTAL). Append-only: hanya mengisi 0/null. Termasuk
        # pl_package_count yang recheck Gemini sengaja TIDAK backfill (tidak
        # bisa membedakan sel kosong vs tercetak — text layer bisa).
        # =========================================
        ...
        # SKIP kalau tidak ada PL: tanpa pl_quantity, derivasi ini justru bisa
        # men-nol-kan inv_quantity/inv_amount yang sudah benar dari invoice.
        if has_pl_doc:
        ...
        if not has_pl_doc:
            # COO numeric diturunkan dari PL; tanpa PL, skip supaya coo_* tidak
            # malah ke-nol-kan. (coo_* tetap dijaga guard absent-doc di bawah.)
            print("[COO_NUMERIC_FROM_PL] skipped: no packing list")
        elif _is_coo_aggregate_top_row_vendor(vendor_id):
        ...
        # Validasi PL hanya kalau PL diupload. Tanpa PL, pl_* sengaja null dan
        # tidak boleh memunculkan flag "missing pl_*" / mismatch.
        if has_pl_doc:
