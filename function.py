...
# Vendor yang butuh RECONCILE printed-total: kalau sum(field per baris) !=
# total tercetak (mis. sum(pl_nw) != pl_total_nw), baca ULANG kolom field itu
# PLUS sel TOTAL-nya, lalu TERIMA hasil re-read HANYA kalau internally consistent
# (sum re-read == total re-read). Generic untuk SEMUA pasangan total<->row
# (lihat _PRINTED_TOTAL_RECONCILE_SPECS) — bukan hanya NW/GW.
#
# Beda dari recheck total-issue yang sudah ada (_apply_detail_line_recheck_result
# greedy gate):
#  - jalan untuk vendor label-only (yang TIDAK menulis-balik nilai via recheck),
#  - bisa MEMPERBAIKI sel TOTAL yang ikut salah baca OCR (greedy gate menganggap
#    declared_total = ground truth, padahal bisa salah),
#  - acceptance-nya internal-consistency (sum re-read == total re-read), bukan
#    "mendekati declared_total".
# Penyebab utama liow_ko: halaman PL terakhir (scan kualitas rendah) salah baca
# beberapa sel per baris DAN sel TOTAL.
PRINTED_TOTAL_RECONCILE_VENDORS = {
    "liow_ko",
}

def _should_reconcile_printed_totals(vendor_id: str) -> bool:
    return normalize_vendor_id(vendor_id) in PRINTED_TOTAL_RECONCILE_VENDORS
...
#
# Pasangan (declared_total <-> per-row field). doc -> anchor + instruksi
# dokumen; col -> label kolom untuk prompt.
#
# Sengaja DIKECUALIKAN:
#  - inv_amount: derived (di-recompute = inv_quantity * inv_unit_price oleh
#    _generate_inv_amount_before_validation). Mismatch amount invoice ditangani
#    lewat inv_quantity; amount mengikuti.
#  - pl_volume: matching-nya conversion-aware (M3/CUFT), bukan equality.
# =========================================================================
_PRINTED_TOTAL_RECONCILE_SPECS = [
    {"total": "pl_total_nw",        "row": "pl_nw",            "doc": "pl",  "col": "NW (Net Weight)"},
    {"total": "pl_total_gw",        "row": "pl_gw",            "doc": "pl",  "col": "GW (Gross Weight)"},
    {"total": "pl_total_quantity",  "row": "pl_quantity",      "doc": "pl",  "col": "QUANTITY"},
    {"total": "pl_total_package",   "row": "pl_package_count", "doc": "pl",  "col": "TOTAL CTN"},
    {"total": "inv_total_quantity", "row": "inv_quantity",     "doc": "inv", "col": "QUANTITY"},
]


def _build_printed_total_reconcile_payload(rows: list, spec: dict) -> list:
    """
    Baris yang dikirim untuk di-baca-ulang. Hanya baris item NYATA:
    skip baris TRUE / CHILD PO / secondary PO-split (yang field aditifnya
    sengaja 0). Anchor diambil dari dokumen yang sesuai (pl_* / inv_*).
    """
    doc = spec["doc"]
    payload = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        ms = str(row.get("match_score", "")).strip().upper()
        if ms in ("TRUE", "CHILD PO"):
            continue
        if row.get("_po_split_primary") is False:
            continue
        rn = _safe_row_no_int(row)
        if rn is None:
            continue

        if doc == "pl":
            po = row.get("pl_customer_po_no")
            item = row.get("pl_item_no") or row.get("pl_vendor_article_no")
            qty = row.get("pl_quantity")
            desc = row.get("pl_description")
        else:
            po = row.get("inv_customer_po_no")
            item = row.get("inv_spart_item_no") or row.get("inv_vendor_article_no")
            qty = row.get("inv_quantity")
            desc = row.get("inv_description")
...
def _extract_printed_total_reconcile_column(
    file_uri: str,
    spec: dict,
    payload_rows: list,
    vendor_id: str = "default",
    ...
    SEKALIGUS membaca grand total kolom itu yang TERCETAK di baris TOTAL.

    Return dict {"rows": [{_detail_row_no, value}], "total": number}.
    {} kalau gagal / format tak terduga.
    """
    if not payload_rows:
        return {}

    doc_label = "PACKING LIST" if spec["doc"] == "pl" else "INVOICE"
    col = spec["col"]
    rows_json = json.dumps(payload_rows, ensure_ascii=False, indent=2)

    prompt = (
        f"ROLE:\n"
        f"Anda verifier angka KHUSUS kolom \"{col}\" pada dokumen {doc_label}.\n\n"
        "KONTEKS:\n"
        "ROWS di bawah adalah baris item hasil ekstraksi. Sum nilai kolom ini per baris\n"
        f"TIDAK SAMA dengan grand total \"{col}\" yang tercetak. Penyebab umum: sebagian\n"
        "sel salah baca OCR (mis. halaman terakhir scan kualitas rendah), dan KADANG sel\n"
        "TOTAL ikut salah baca.\n\n"
        "TUGAS:\n"
        f"1) Untuk SETIAP row, temukan baris item-nya di tabel {doc_label} memakai anchor:\n"
        "   po_no (kolom paling kiri / Purchase order Number) + item_no (PART NUMBER) + quantity.\n"
        f"   Lalu baca ULANG nilai kolom \"{col}\" pada baris itu -> field \"value\".\n"
        f"2) Baca grand total \"{col}\" yang TERCETAK pada baris TOTAL paling bawah tabel -> field \"total\".\n\n"
        "ATURAN BACA (KETAT):\n"
        f"- HANYA baca kolom \"{col}\". JANGAN ambil kolom lain (quantity/carton/dll) sebagai value.\n"
        "- BACA digit per digit PERSIS seperti tercetak (umumnya 2 desimal). JANGAN membulatkan:\n"
        "  kalau tercetak 35.60 tulis 35.60 (BUKAN 36.0); 6.20 tetap 6.20 (BUKAN 6.28).\n"
        "- JANGAN menyalin nilai dari baris tetangga (atas/bawah). Tiap baris punya angkanya\n"
        "  sendiri; kalau baris atas 4.44 lalu baris ini tercetak 0.87, ambil 0.87.\n"
        "- Kalau header kolom tidak tercetak ulang di halaman lanjutan, URUTAN KOLOM TETAP SAMA;\n"
        "  baca posisional.\n"
        "- Kembalikan SEMUA row (jumlah & urutan sama dengan ROWS), pertahankan _detail_row_no.\n"
        "- Kalau sebuah angka benar-benar tidak terbaca, isi 0.\n\n"
        "OUTPUT (HANYA JSON object valid, tanpa teks lain):\n"
        "{\"rows\": [{\"_detail_row_no\": number, \"value\": number}], \"total\": number}\n\n"
        "ROWS:\n"
        f"{rows_json}"
    )
...
def _reconcile_printed_totals(
    ...
    sum-per-baris-nya != total tercetak: baca ULANG kolom itu + sel TOTAL,
    lalu TERIMA hasil re-read HANYA kalau:
      (1) internally consistent: sum(re-read per baris) == total re-read, DAN
      (2) sane band: total re-read tidak "ngarang jauh" dari sum lama / declared.

    Kalau diterima: tulis-balik nilai baris yang berubah + perbaiki declared
    total (header_obj/base_header_obj/rows) ke total re-read — sebab sel TOTAL
    pun bisa salah baca OCR. Kalau tidak konsisten -> ROLLBACK (mismatch tetap
    muncul untuk review manual). Konservatif: tidak pernah memaksa angka.
    """
    ...
    def _col_sum(field):
        return sum(_to_float(r.get(field)) or 0.0 for r in rows if isinstance(r, dict))

    def _set_declared_total(field, value):
        # _apply_header_to_rows (final) menimpa total_* dari header_obj, jadi
        # WAJIB update header juga, bukan cuma rows.
        if isinstance(header_obj, dict):
            header_obj[field] = value
        if isinstance(base_header_obj, dict):
            base_header_obj[field] = value
        for r in rows:
            if isinstance(r, dict):
                r[field] = value
    ...
    for spec in _PRINTED_TOTAL_RECONCILE_SPECS:
        total_field = spec["total"]
        row_field = spec["row"]

        declared = _to_float(_first_non_null_nonzero(rows, total_field))
        if declared is None:
            continue

        sum_cur = _col_sum(row_field)
        if abs(sum_cur - declared) <= EPS:
            continue  # tidak mismatch -> tidak perlu re-read (no Gemini call)

        payload = _build_printed_total_reconcile_payload(rows, spec)
        if not payload:
            continue

        print(
            f"[PRINTED_TOTAL_RECONCILE] {row_field} mismatch "
            f"(sum={sum_cur:.4g}, doc={declared}) -> re-read kolom"
        )
        ...
        overrides = {}
        for it in reread.get("rows") or []:
            if not isinstance(it, dict):
                continue
            rn = _safe_row_no_int(it)
            if rn is None:
                continue
        ...
        reread_total = _to_float(reread.get("total"))
        if reread_total is None:
            continue

        # trial sum: mirror validasi (sum SEMUA baris), override yang di-reread.
        trial = 0.0
        for r in rows:
            if not isinstance(r, dict):
                continue
            rn = _safe_row_no_int(r)
            if rn is not None and rn in overrides:
                trial += overrides[rn]
            else:
                trial += _to_float(r.get(row_field)) or 0.0

        # ACCEPTANCE GATE.
        consistent = abs(trial - reread_total) <= EPS
        sane = (
            abs(reread_total - sum_cur) <= max(1.0, 0.05 * abs(sum_cur))
            or abs(reread_total - declared) <= max(1.0, 0.05 * abs(declared))
        )

        if not (consistent and sane):
            print(
                f"[PRINTED_TOTAL_RECONCILE] {row_field} ROLLBACK "
                f"(trial={trial:.4g} vs reread_total={reread_total}; "
        ...
        for r in rows:
            if not isinstance(r, dict):
                continue
            rn = _safe_row_no_int(r)
            if rn is None or rn not in overrides:
                continue
            old = _to_float(r.get(row_field))
            new = overrides[rn]
            if old is not None and abs(old - new) <= 1e-9:
                continue
            print(f"[PRINTED_TOTAL_RECONCILE] {row_field} row_no={rn}: {old} -> {new}")
            r[row_field] = new
            changed += 1

        _set_declared_total(total_field, reread_total)
        total_fixed += 1
        print(
            f"[PRINTED_TOTAL_RECONCILE] {row_field} ACCEPTED: {changed} baris diperbaiki, "
            f"{total_field} -> {reread_total}"
        )

    if total_fixed:
        print(f"[PRINTED_TOTAL_RECONCILE] selesai: {total_fixed} kolom direkonsiliasi")

    return rows


def _run_printed_total_reconcile_pass(
    file_uri: str,
    rows: list,
    header_obj: dict,
    base_header_obj: dict,
    ...
    if not _should_reconcile_printed_totals(vendor_id):
        return rows
    try:
        rows = _reconcile_printed_totals(
            file_uri=file_uri,
            rows=rows,
        ...
        # Jangan gagalkan OCR utama hanya karena pass tambahan gagal.
        print(f"[PRINTED_TOTAL_RECONCILE][WARN] skipped: {e}")
    return rows
...
def run_ocr(
    ...
    try:
        ...
        # =========================================
        # PRINTED-TOTAL RECONCILE (generic, additive)
        # Kalau sum(field) per baris != total tercetak, baca ULANG kolom itu
        # + sel TOTAL, terima HANYA kalau internally consistent (sum re-read ==
        # total re-read) + sane band. Menangani kasus liow_ko: halaman PL
        # terakhir (scan kualitas rendah) salah baca sel NW/GW per baris DAN
        # sel TOTAL — yang tidak tertangani backfill zeroed-only / recheck
        # label-only. Generic untuk semua pasangan total<->row
        # (_PRINTED_TOTAL_RECONCILE_SPECS), gated PRINTED_TOTAL_RECONCILE_VENDORS.
        # Sebelum _generate_inv_amount_before_validation supaya fix inv_quantity
        # ter-recompute ke inv_amount. Update header_obj/base_header_obj juga
        # karena _apply_header_to_rows (final) menimpa total_* dari header.
        # =========================================
        all_rows = _run_printed_total_reconcile_pass(
            base_detail_input_uri,
            all_rows,
            header_obj=header_obj,
            base_header_obj=base_header_obj,
