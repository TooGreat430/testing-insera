...
- Header utama line item:
  MARKS | DESCRIPTION | SPART / CPART | QTY & UNIT | UNIT PRICE (USD) | AMOUNT (USD)
- Dalam satu line item biasanya ada:
  1) product/material code di awal block
  2) description multi-line
  3) SPART / CPART
  4) quantity + unit
  5) marks block: PT.IS / P/O No.... / SURABAYA / optional PLT NO.... / CTN NO....
- Pada vendor SHIMANO, item yang sama bisa muncul berulang sebagai beberapa printed row terpisah dengan CTN/PLT berbeda.
- Jika dokumen mencetaknya sebagai block/row terpisah, anggap itu item terpisah dan JANGAN merge hanya karena description atau spart sama.
...
1. inv_customer_po_no
   ...
   - Contoh:
     - "P/O No.45324353" -> inv_customer_po_no = "45324353"
     - "P/O No. 45322131" -> inv_customer_po_no = "45322131"
   - Jangan ambil: invoice number, LC number, CTN NO, PLT NO
...
3. inv_spart_item_no
   - HANYA ambil dari kolom "SPART / CPART" pada row item yang sama.
   - JANGAN AMBIL dari kolom DESCRIPTION.
   - JANGAN AMBIL product/material code di awal block description.
   - Jika value berbentuk "AMT401EJHFPRX085 / AMT401EJHFPRX085", ambil 1 nilai spart saja: "AMT401EJHFPRX085"
   - Jika kiri dan kanan berbeda, ambil value sebelah kiri slash.
   - inv_spart_item_no tidak boleh diawali angka.
   - Jika tidak ada bukti jelas pada kolom "SPART / CPART", isi "null".
...
4. inv_description
   - Ambil deskripsi barang dari kolom DESCRIPTION saja.
   - Gabungkan seluruh line description item menjadi satu string utuh.
   - Jangan masukkan product/material code, SPART/CPART, PT.IS, P/O No., SURABAYA, PLT NO., CTN NO., quantity, unit price, amount.
   - Normalisasi kata pecah bila perlu.
...
7. inv_quantity_unit
   - Ambil unit yang menempel pada quantity: PCS, SET, dll.

8. inv_unit_price
   - Ambil dari kolom "UNIT PRICE (USD)". Nilai numeric saja.

9. inv_amount
   - Ambil dari kolom "AMOUNT (USD)". Nilai numeric saja.
"""

SHIMANO_SINGAPORE_PL_PROMPT = """
PACKING LIST (PL)
...
- Satu item packing list biasanya terdiri dari:
  ...
  4) qty & unit
  5) marks block: PT.IS / P/O No.... / SURABAYA / optional PLT NO.... / CTN NO....
  6) beberapa detail packing component
  7) satu total line item
- Satu printed block item adalah satu item packing list.
...
2. pl_item_no
   - HANYA ambil dari kolom "SPART / CPART" pada row item yang sama.
   - JANGAN AMBIL dari kolom DESCRIPTION.
   - Jika value berbentuk "AMT401EJHFPRX085 / AMT401EJHFPRX085", ambil "AMT401EJHFPRX085"
   - Jika candidate diawali angka, tolak.
   - Jika tidak ada bukti jelas pada kolom "SPART / CPART", isi "null".

3. pl_description
   - Ambil deskripsi barang dari kolom DESCRIPTION saja.
   - Jangan masukkan product/material code, SPART/CPART, PT.IS, P/O No., SURABAYA, PLT NO., CTN NO., detail packing, total line, angka NW/GW/volume.

4. pl_quantity
   - Ambil value numerik quantity dari kolom "QTY & UNIT".

5. pl_package_unit
   - HANYA boleh dari kolom "PACKING TYPE"
   - Canonical value: ["CT", "PX", "BL", "PXCT", "null"]
   - Carton/Full Carton/Loose Carton/CTN -> "CT"
   - Pallet/PLT -> "PX"
   - Bale -> "BL"
   - Jika lebih dari 1 package unit -> "PXCT"

6. pl_package_count
   - Ambil jumlah total line package untuk item tersebut.
   - Aturan hitung dari total line:
     - "1 Carton" -> 1, "5 Carton" -> 5, "1 Pallet & 6 Carton" -> 7

7. pl_nw
   - Ambil net weight item-level dari total line sel sebelum tanda '/' di kolom NET/GROSS WEIGHT.

8. pl_gw
   - Ambil gross weight item-level dari total line sel setelah tanda '/' di kolom NET/GROSS WEIGHT.

9. pl_volume
   - Ambil volume item-level dari total line item di kolom DIMENSION (CM, M3).
"""

SHIMANO_SINGAPORE_BL_PROMPT = """
BILL OF LADING (BL)

Struktur umum BL SHIMANO (SINGAPORE):
- Page 1 adalah ocean bill of lading shipment summary.
- Page 2 adalah "B/L ATTACHMENT" dan ini yang berisi item/category-level description.
- Pada page 1, deskripsi utama hanya bersifat umum seperti "AS PER ATTACHED LIST".
- Pada page 2 attachment, item-level category yang muncul antara lain:
  - FRONT DERAILLEUR HS NUMBER: 8714.99
  - REAR DERAILLEUR HS NUMBER: 8714.99
  - SHIFT LEVER HS NUMBER: 8714.99
  - SHIFT/BRAKE LEVER HS NUMBER: 8714.99
  - CASSETTE SPROCKET HS NUMBER: 8714.93
- BL SHIMANO bersifat summary-level, bukan detail spart/item penuh.

1. bl_description dan bl_hs_code:
   - Ekstrak langsung dari halaman 2 (B/L ATTACHMENT).
   - bl_description adalah teks sebelum "HS NUMBER:".
   - bl_hs_code adalah nilai setelah "HS NUMBER:".
   - Hanya boleh mengambil dari dokumen Bill Of Lading (BL), TIDAK BOLEH dari dokumen yang lain.

   Contoh:
   - "FRONT DERAILLEUR HS NUMBER: 8714.99" -> bl_description = "FRONT DERAILLEUR", bl_hs_code = "8714.99"
   - "REAR DERAILLEUR HS NUMBER: 8714.99" -> bl_description = "REAR DERAILLEUR", bl_hs_code = "8714.99"
   - "SHIFT LEVER HS NUMBER: 8714.99" -> bl_description = "SHIFT LEVER", bl_hs_code = "8714.99"

   - Satu item kategori BL dapat diextrak beberapa kali sesuai jumlah item pada halaman 2.
   - Jika tidak ada item kategori di BL, bl_description dan bl_hs_code = null.
"""

SHIMANO_SINGAPORE_COO_PROMPT = """
CERTIFICATE OF ORIGIN (COO)

Struktur umum COO SHIMANO (SINGAPORE):
- COO bisa berbentuk: ORIGIN DECLARATION (AWSC), DECLARATION OF ORIGIN (RCEP), BACK-TO-BACK DECLARATION OF ORIGIN
- Producer country bisa berbeda: INDONESIA, MALAYSIA, CHINA
- Satu item COO umumnya berisi urutan:
  1) marks block: PT.IS / P/O No.... / SURABAYA / optional PLT NO.... / CTN NO....
  2) INVOICE NUMBER
  3) INVOICE DATE
  4) product/material code
  5) short code / model code
...
2. coo_mark_number
   - Ambil seluruh isi marks block dari kolom "MARKIN" sebagai satu string ter-normalisasi.
   - Sertakan: PT.IS / P/O No.... / SURABAYA / PLT NO.... / CTN NO....
   - Contoh: "PT.IS P/O No.45324415 SURABAYA PLT NO.1-1 CTN NO.2-22"
   - Jika marks block tidak ada, isi "null".

3. coo_description
   - Ambil hanya deskripsi barang naratif.
   - Gabungkan semua line description item menjadi satu string utuh.
   - Jangan masukkan marks block, INVOICE NUMBER, INVOICE DATE, product code, SPART code, HS CODE, ORIGIN REF NO, DATE OF ISSUANCE, PRODUCER AUTHORISATION CODE, COUNTRY OF FIRST EXPORTING PARTY, criterion, quantity, amount.

4. coo_hs_code
   - Ambil dari line "HS CODE:".

5. coo_quantity
   - Ambil value numerik quantity dari kolom "QUANTITY".
...
8. coo_package_unit
   - Ambil dari marking block.
   - Canonical: ["CT", "PX", "BL", "PXCT", "null"]
   - hanya CTN NO. -> "CT", hanya PLT NO. -> "PX", keduanya -> "PXCT"
...
10. coo_amount
   - Ambil FOB value dari kolom "FOB VALUE (USD)". Angka numeric saja.

11. coo_criteria
   - Ambil dari kolom "ORIGIN CONFERING CRITERION".
   - Jangan masukkan persentase, country, unit, quantity, atau amount.
   - Contoh: "RVC 100% CHINA PCS 2650 $ 4,929.00" -> coo_criteria = "RVC"
