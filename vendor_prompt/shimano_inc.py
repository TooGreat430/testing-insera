...
6. `pl_package_count`:
    ...
    - ATURAN SANGAT PENTING: Jika pada dokumen terdapat dua value dengan UNIT yang beda yang tergabung dalam satu UNIT dengan satuan yang lebih besar, seperti:
      ...
      (       1 C/T)
      Maka:
      pl_package_count untuk line item tersebut adalah 2 P/T + 6 C/T + 1 C/T = 2 + 6 + 1 = 9.
...
7. `pl_nw`:
    - Ekstrak nilai angka dari kolom "Net Weight" baris atas yang tidak ada simbol "@".
    - Apabila pada 1 line item terdapat beberapa baris dengan nilai "Net Weight", maka jumlahkan semua nilai angka tersebut.

8. `pl_gw`:
    - Ekstrak nilai angka dari kolom "Gross Weight" baris atas.
    - Apabila pada 1 line item terdapat beberapa baris dengan nilai "Gross Weight", maka jumlahkan semua nilai angka tersebut.

9. `pl_volume`:
    - Ekstrak nilai angka dari kolom "Measure" baris atas (misalnya dari "0.080M3", ekstrak 0.080).
    - Apabila pada 1 line item terdapat beberapa baris dengan nilai "Measure", maka jumlahkan semua nilai angka tersebut.
...
SHIMANO_INC_BL_PROMPT = """
BILL OF LADING (BL):

1. `bl_description`:
    - Ekstrak deskripsi barang langsung dari dokumen BL.
    - Ambil dari area description pada BL page 2 jika tersedia.
    - Hanya boleh mengambil dari dokumen BL, TIDAK BOLEH dari dokumen yang lain.
    - Contoh:
      FRONT CHAINWHEEL 165MM HS NUMBER: 8714.96
      FRONT CHAINWHEEL 170MM HS NUMBER: 8714.96
      FRONT DERAILLEUR HS NUMBER: 8714.99
      REAR DERAILLEUR HS NUMBER: 8714.96
      DISC BRAKE HS NUMBER: 8714.94
      maka bl_description: "FRONT CHAINWHEEL 165MM", "FRONT CHAINWHEEL 170MM", dll.
    - Jika tidak ada item ditemukan, isi null.

2. `bl_hs_code`:
    - Ambil HS code yang terkait dengan bl_description yang sama.
    - Hanya boleh mengambil dari dokumen Bill Of Lading (BL), TIDAK BOLEH dari dokumen yang lain.
...
3. `bl_mark_number`:
    ...
    - Contoh:
      ...
      MADE IN JAPAN
      CTN No. -> 1-5, 13, 33-80
      Maka bl_mark_number: "PT. IS PO# 43018041 C/T No.: 1-5, 13, 33-80 MADE IN JAPAN"
"""

SHIMANO_INC_COO_PROMPT = """
CERTIFICATE OF ORIGIN (COO):
1. `coo_seq`:
   - Ambil dari kolom "Item number".
   - Nilai numeric.
   - Item number tercetak jelas seperti: 1, 2, 3, ...
2. `coo_mark_number`:
    - Ekstrak dari "7. Marks and numbers on packages".
    - Apabila tidak ada informasi marks and numbers pada kolom 7 atau tertulis "N/M" (Not Mentioned), maka biarkan null.
3. `coo_description`: Ekstrak deskripsi teks dari kolom "6. Description of goods" (ambil murni deskripsi barangnya saja, misal "SMALL PARTS SMMA-R 160 D/D", abaikan teks "Invoice No", "PO No", dan "PART#").
4. `coo_hs_code`: Ekstrak dari kolom "7. HS Code".
5. `coo_package_count`: Biarkan null.
6. `coo_package_unit`: Biarkan null.
